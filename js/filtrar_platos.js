let platosDelMenu = [
  plato1 = {
    ...
    descripcion: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea quasi dolores, nemo deleniti voluptatum quas?",
    tipo: "almuerzo",
    precio: "$25",
    nombreArchivo: "1.jpg",
  },
...
function constructorDePlato(plato) {
  elemento = `
  <div class="container_plato" >
    <img src="./assets/images/menu/${plato.nombreArchivo}">
    <div class="plato_description">
      <h2>${plato.plato}</h2>
      <p class="raiting">★★★★★</p>
...
function construirMenu(menu) {
  ...
  let platos = []
  menu.forEach( plato => {
    platos.push(constructorDePlato(plato))
  })
  platos.forEach(plato => {
    section2.innerHTML += plato
  })
  const platosNodeList = document.querySelectorAll('.container_plato')
  const platosDelDom = [...platosNodeList]
  const platosPromesas = [] 

  platosDelDom.forEach( plato => {
    const printDish = () => {
      return new Promise ((resolve, reject) => {
        true
        ? setTimeout(() => {resolve(plato)}, 75)
        : reject(new Error)
      })
    }
    platosPromesas.push(printDish)
  })
  const printDishAsync = async () => {
    for (let i = 0; i < platosPromesas.length; i++) {
      await platosPromesas[i]().then(plato => { plato.classList.add('appear')})
    }
  }
  setTimeout(() => {
    printDishAsync()
